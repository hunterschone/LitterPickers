...
options=setIfUnset(options,'screen',1);
...
%% check if second screen is present when requested

if options.screen > 1
    monpos = get(0,'MonitorPositions');
    if size(monpos,1) < options.screen
        error('Chosen monitor %i is not available. Please make sure it is plugged in and start Matlab only *after* the monitor is in.')
    end
end
...
while minEvidenceWeight<minRequiredEvidenceWeight &&... 
    ...
    if simulationMode
        ...
        [objectPositions,distMat_ltv]=letSubjectArrangeObjects_circularArena(imageData(cTrial_objectIs),instructionString,options);
    end
